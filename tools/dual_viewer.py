...
# サーマル MLX90640。ファーム側(thermal_mlx90640.c rotate_crop)で
# 「90度右回転＋中央24行crop」済みの 24×24 が送られてくる。
# 受信側は reshape(24, 24) するだけ。回転・crop・上下反転は一切行わない。
T_ROWS = 24
T_COLS = 24
T_PIXELS = T_ROWS * T_COLS  # 576
...
# バイナリサーマルフレーム（ファームが回転＋24×24crop済み = 576画素を送る）:
#   生:   magic(0xAA 0x55) + Ta(int16 LE) + 576×int16 LE = 1156B
#   前景: magic(0xAA 0x56) +            + 576×int16 LE = 1154B（背景差分後の正の差分）
BIN_MAGIC = b"\xAA\x55"
BIN_FRAME_LEN = 2 + 2 + T_PIXELS * 2     # 1156（生）
BIN_FG_MAGIC2 = 0x56
BIN_FG_FRAME_LEN = 2 + T_PIXELS * 2      # 1154（前景）
        ...
        while plt.fignum_exists(fig.number):
            ...
            if data:
                ...
                if latest_t is not None:
                    ta, pix = latest_t
                    # ファームが向き補正済みを送るので reshape(24,24) するだけ。加工しない。
                ...
                if latest_d is not None:
                    dd, ss = latest_d
                    # ファームが向き補正(左右反転)済みを送るので reshape のみ。加工しない。
