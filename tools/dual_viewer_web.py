...
class SharedState:
    ...
    def set_saved(self, counts):
        """ 保存枚数カウンタをディスク実数で上書きする（削除・起動同期用）。 """
        with self._lock:
            self.rec_saved = {"crow": counts.get("crow", 0),
                              "not_crow": counts.get("not_crow", 0)}
...
def count_saved_by_label(out_dir):
    """ out_dir 内の .npz をラベル別に集計して {"crow": n, "not_crow": m} を返す。
        ディスク上の実数を真実とし、起動時・削除後のカウンタ同期に使う。 """
    counts = {"crow": 0, "not_crow": 0}
    if not os.path.isdir(out_dir):
        return counts
    for f in sorted(os.listdir(out_dir)):
        if not f.endswith(".npz"):
            continue
        try:
            with np.load(os.path.join(out_dir, f), allow_pickle=True) as z:
                label = str(z["label"])
        except Exception:  # 壊れたファイル等は無視
            continue
        counts[label] = counts.get(label, 0) + 1
    return counts


def delete_samples_by_label(out_dir, label):
    """ out_dir 内の指定ラベルの .npz を全削除し、削除件数を返す。
        Start/Stop と同じ収集ディレクトリを対象にする（連番は再収集時に再採番）。 """
    deleted = 0
    if not os.path.isdir(out_dir):
        return 0
    for f in sorted(os.listdir(out_dir)):
        ...
        try:
            with np.load(path, allow_pickle=True) as z:
                if str(z["label"]) != label:
                    continue
        except Exception:  # 読めないファイルは削除対象にしない
            continue
        try:
            os.remove(path)
            deleted += 1
    ...
    while not stop_evt.is_set():
        ...
        for kind, payload in msgs:
            ...
            if line.startswith("DET,"):
                ...
                # 先頭5値(cand..d_area)だけ使う。複合判定の追加3値は後方互換で無視。
                parts = line.strip().split(",")
                if len(parts) >= 6:
                    try:
                        state.update_det(tuple(int(x) for x in parts[1:6]))
                    except ValueError:
                        pass
                continue
...
def build_app(args, state, ser):
    ...
    def logo_data_uri():
        """ ロゴ SVG を base64 data URI にして返す（assets 不要で確実に埋め込む）。
            リポジトリルート（このファイルの2階層上）の images/logo を起点に探す。 """
        import base64
        repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        path = os.path.join(repo_root, "images", "logo",
                            "terraguard-ai-logo-light.svg")
        try:
            with open(path, "rb") as f:
                b64 = base64.b64encode(f.read()).decode("ascii")
            return f"data:image/svg+xml;base64,{b64}"
        except OSError:
            return None  # 見つからなければヘッダはテキストにフォールバック

    _logo_uri = logo_data_uri()
    header = (html.Img(src=_logo_uri, alt="TerraGuard AI",
                       style={"height": "50px", "margin": "8px 0"})
              if _logo_uri else html.H3("TerraGuard AI Demo"))
    ...
    app.layout = html.Div(
        ...
        children=[
                                        ...
                                        children=[
                                            ...
                                            html.Button(
                                                ...
                                                style={"fontSize": "14px",
                                                       ...
                                                       "cursor": "pointer"}),
                                            # 選択中ラベルの収集サンプルを一括削除。
                                            # 誤操作防止に二段階（1押し目=確認、2押し目=実行）。
                                            html.Button(
                                                "Delete", id="btn-collect-delete",
                                                n_clicks=0,
                                                style={"fontSize": "14px",
                                                       "padding": "6px 16px",
                                                       "background": "#c0392b",
            ...
            # 削除の二段階確認用: どのラベルが「確認待ち」かを保持（None=未武装）。
            dcc.Store(id="delete-armed", data=None),
        ],
    )
    ...
    # 収集サンプルの削除（選択中ラベルを一括）。二段階確認:
    #   1押し目 … 確認メッセージを出し、armed=現ラベルにする（まだ消さない）。
    #   2押し目（同ラベル）… 実際に削除し、カウンタをディスク実数で同期。
    #   ラベルを変えると武装解除（別ラベルを誤って消さないように）。
    @app.callback(
        Output("delete-msg", "children"),
        Output("delete-armed", "data"),
        Input("btn-collect-delete", "n_clicks"),
        State("collect-label", "value"),
        State("delete-armed", "data"),
        prevent_initial_call=True,
    )
    def on_collect_delete(_n, label, armed):
        if armed != label:
            # 1押し目（または別ラベルで武装中）: 確認を促すだけ。
            n = count_saved_by_label(args.collect_out).get(label, 0)
            if n == 0:
                return f"No '{label}' samples to delete.", None
            return (f"Delete {n} '{label}' sample(s) in {args.collect_out}? "
                    f"Press Delete again to confirm."), label
        # 2押し目（同ラベル）: 実行。
        deleted = delete_samples_by_label(args.collect_out, label)
        counts = count_saved_by_label(args.collect_out)
        state.set_saved(counts)
        # 連番も既存ファイルの続きに採り直す（次の収集が穴を埋めないように）。
        state._collect_idx = next_sample_index(args.collect_out)
        return f"Deleted {deleted} '{label}' sample(s).", None
...
def main():
    ...
    state._collect_idx = next_sample_index(args.collect_out)
    # 既存サンプルのラベル別実数を初期カウントに反映（UI表示・削除と整合）。
    state.set_saved(count_saved_by_label(args.collect_out))
    if state._collect_idx:
        init = state.rec_saved
        print(f"収集: {args.collect_out} に既存 {init['crow']}/crow "
              f"{init['not_crow']}/not_crow → 連番 {state._collect_idx:06d} から")
