...
- **出力フォーマット**: 距離はテキスト行 `DIST,<z0..z63>` / `STAT,<s0..s63>`、サーマルは**バイナリフレーム** `0xAA 0x55 + Ta(int16 LE) + 576×int16 LE`（計1156B、PRINTF の書式変換コストを避け高速化）。サーマル前景（背景差分の正側）は `0xAA 0x56 + 576×int16 LE`。`tools/dual_viewer.py` がバイナリ＋テキスト混在を `extract_messages` で分離して2画面表示する。
  - **解像度は 24×24（576画素）**: MLX90640 の生は 32×24=768 だが、ファームは**取得直後に `rotate_crop()`（90度右回転＋中央24行crop）で 24×24 に整える**（センサを物理的に90度回転して取り付けているため）。**送出・統計・背景差分・NPU入力・データ収集まで、すべての処理は 24×24 で行われる**。シリアルに流れるサーマルフレームも 576×int16 である。

### CLI でのビルド・書き込み・シリアル確認

> 🛠️ **実行コマンドは `frdm-mcxn947-dev` スキルに一本化**してある。ビルド・書き込み・
> シリアル確認・west サンプルビルドを行うときは、まず `frdm-mcxn947-dev` スキルを起動し、
> その手順（確定パス／ビルド／書き込み／シリアル確認）に従うこと。VS Code を使わず
> ターミナルだけで完結できる。以下は手順そのものではなく、運用上の**重要な前提知識**。

- **ビルド**: CMake Presets（`debug`/`release`）。`prj.conf` 変更時のみ configure を再実行。
  成果物は `src/FRDM-MCXN947/terra-guard-ai/debug/terra-guard-ai_cm33_core0.elf`。
- **書き込み**: LinkServer で上記 ELF を flash。`LinkServer` に単体の `reset` サブコマンドは無い。
- **シリアル確認**（最重要）: ⚠️ **必ず `921600` で開くこと**。本ファームの VCOM は `board.h` の
  `BOARD_DEBUG_UART_BAUDRATE=921600U`（MLX90640 の大フレームを捌くため 115200 の8倍）。
  **115200 で開くと全バイト文字化けし「ファーム異常」「センサ無反応」と誤診しやすい。**
  受信側ツール（`dual_viewer.py` / `dual_viewer_web.py` / `fps_meter.py`）のデフォルトも 921600。
  （※ core1 のデバッグUARTのみ 115200。hello_world で経路切り分けする際も hello_world 側は 115200。）
  サーマルはバイナリなので `screen` では文字化けする。pyserial で時間窓読みするのが基本。
- **起動直後の1回きりの出力（I²Cスキャン等）を拾うコツ**: 後からシリアルを開くと取り逃す。
  **先に受信をバックグラウンド開始 → その後 flash** すると再起動時の起動ログを最初から拾える。
- ⚠️ **flash 中に VCOM が切断される環境がある**: flash 中にシリアルを開いたままだと MCU-Link が
  一旦 USB から消え `Device not configured` で読み取りが落ちる（無音=ハングは誤り）。
  定常出力（DIST/STAT 等）を見たいときは **flash 完了 → ポート再列挙を待つ → 改めて開く** 手順が安定。

### west で SDK サンプルを直接ビルド（動作確認に便利）

新機能はまず公式サンプルを west で焼いて疎通確認すると早い（例: 温度センサ）。
**実行コマンドは `frdm-mcxn947-dev` スキルを参照。** 注意点だけ記す:
システムPython（3.14等）には yaml が無くビルドが失敗するため、必ず MCUXpresso venv の
Python/west を PATH 先頭に入れること（`ARMGCC_DIR` の設定も必要）。
...
センサ生は 32×24=768 画素の校正済み温度[℃]を取得し、**取得直後に `rotate_crop()` で 24×24=576 に整えてから**シリアル出力・背景差分・NPU入力へ渡す（以降の処理はすべて 24×24。[datasheets/MLX90640.md](./datasheets/MLX90640.md) に詳細）。
