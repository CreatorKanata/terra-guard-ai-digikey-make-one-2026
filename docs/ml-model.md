...
>
> **⚠️ 解像度に関する重要事項**: MLX90640 の**生**出力は 32×24 だが、ファームは**取得直後に
> `rotate_crop()`（90度右回転＋中央24行crop）で 24×24 に整える**。背景差分・前景・統計・
> バイナリ送出・**NPU入力・データ収集まで、処理パイプラインはすべて 24×24** で統一されている。
> 距離 8×8 は最近傍3倍 kron で 24×24 に拡大して束ねる。よって本文中に残る「32×24」「[1,24,32,C]」
> は §1〜§3.4 の**旧調査記録**であり、確定実装ではない（§3.5 が正）。
...
[5] 組み込み (Mac, CLI)    tflm_cifar10 を雛形に model_data.h と op resolver を差し替え、
        ...
        ▼
[6] ビルド・書き込み       ファームをビルドして書き込み（frdm-mcxn947-dev スキル）
        │
        ▼
...
> ⚠️ **本節は旧調査記録（基準グリッドを 32×24 としていた頃）。確定実装は §3.5：基準グリッドは
> 24×24（センサ生 32×24 を取得直後に `rotate_crop` で 24×24 化したもの）、入力は `[1,24,24,4]`。**
...
> 🛠️ **実行コマンドは `frdm-mcxn947-dev` スキルに一本化**してある。再学習・収集・変換・
> ビルド・書き込みを行うときは、まず `frdm-mcxn947-dev` スキルを起動し、その「ML / NPU
> ワークフロー」節の手順に従うこと（このドキュメントには各ステップの**意味**だけを記す）。

ツール（すべて `tools/ml/`、依存は `tools/.venv`／学習のみ `tools/ml/.venv` で arm64）と
各ステップの役割:

1. **収集** — `collect_dataset.py`（CLI・キー打鍵ラベリング c=crow/n=not_crow）または
   `dual_viewer_web.py` の **Start/Stop/Delete UI**。サーマル1フレーム到着を基準に
   「サーマル24×24 / サーマル前景 / 距離8×8 / 距離前景 / DET」を 1 サンプル=1 `.npz` に
   束ねて `dataset/raw/` へ連番保存（向きはファームで確定済みなので無加工で保存）。
   **not_crow には人/腕だけでなく『人もカラスもいない純背景（前景ほぼゼロ）』も十分含める**
   こと（純背景が乏しいと静止シーンが crow に倒れる。2026-06-22 実機で確認）。
...
4. **NPU変換** — `neutron_convert.sh` で mcxn94x 向けに変換（⚠️ **3.0.0 を明示**。
   デフォルトは 3.1.3 を指すので注意。ファーム内蔵ドライバ 3.0.0 と一致させる）。

5. **モデルヘッダ生成＋差し替え** — `make_model_data_h.py` で `model_data.h` を生成し
   `src/FRDM-MCXN947/terra-guard-ai/tflm/pcq_npu/model_data.h` に差し替え。
   op resolver（`model_cifarnet_ops_npu.cpp`）は **Softmax + Slice + NEUTRON_GRAPH の
   3op 固定**で、再変換しても一致するため通常は変更不要。

6. **ビルド・書き込み** — スキルのビルド／書き込み手順に従う（[firmware.md](./firmware.md)）。
...
`tools/ml/neutron_convert.sh`（内部で `docker run --platform linux/amd64 ... neutron-converter
--target mcxn94x --dump-header-file-output`）で変換する。**実行コマンドは `frdm-mcxn947-dev`
スキルの「ML / NPU ワークフロー」節を参照**（3.0.0 を `NEUTRON_SDK_DIR` で明示する点が重要）。
